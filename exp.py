...
class MM_dstb:
    ...
    def fit(self, X, y,capacity_init=None):
        ...
        for i in range(self.n_estimator//self.boost_interval):
            ...
            gbm = lgb.LGBMRegressor(objective=self.custom_mse, max_depth = self.max_depth, learning_rate = self.learning_rate, n_estimators = self.boost_interval, verbosity = -1)
            gbm.fit(X, residuals.flatten())
            self.trees.append(gbm)
            
            # Calculate loss and update capacity
            estimated_delta = gbm.predict(X).reshape(self.t, self.seq, self.k)
            if self.fit_cap:
                self.update_cap(estimated_previous.reshape(-1,self.seq,self.k), estimated_delta.reshape(-1,self.seq,self.k))

            estimated_previous += estimated_delta
            residuals = self.y - np.sum(estimated_previous.reshape(self.t, self.seq, self.k)*self.cap, axis=2, keepdims=True)
    ...
    def update_cap(self,estimated_previous, estimated_delta):
        ...
        for i in range(self.t):
            ...
            dq = delta_t.dot(q_t.T)
            if i == 0 and len(self.history_cap)==0:
                mu = np.sum(self.y[i,:,:])*(delta_t+q_t)
                Sigma = self.k*np.diag(np.sum(estimated_delta[i,:,:]**2,axis=0)) + q_t.dot(q_t.T) + dq + dq.T
            elif i==0 and len(self.history_cap)>0:
                mu = np.sum(self.y[i,:,:])*(delta_t+q_t) + self.lambda_1*self.history_cap[-1][i,:,:].reshape(-1,1)
                Sigma = self.k*np.diag(np.sum(estimated_delta[i,:,:]**2,axis=0) + self.lambda_1/self.k) + q_t.dot(q_t.T) + dq + dq.T
            else:
                mu = np.sum(self.y[i,:,:])*(delta_t+q_t) + self.lambda_1*alpha[i-1,:].reshape(-1,1)
                Sigma = self.k*np.diag(np.sum(estimated_delta[i,:,:]**2,axis=0) + self.lambda_1/self.k) + q_t.dot(q_t.T) + dq + dq.T
            lambda_2 = 1e+5
            # mu = np.concatenate((mu,lambda_2*np.ones((1,1))),axis=0)
            # Sigma = np.concatenate((Sigma,lambda_2*np.ones((1,self.k))),axis=0)
            # alpha[i,:] = nnls(Sigma,mu.flatten())[0].flatten()
            bounds = (0,1)
            try:
                result = lsq_linear(Sigma, mu.flatten(), bounds=bounds)
                alpha[i,:] = result.x.flatten()
            except:
                logging.info(f'time {i} error: SVD did not converge in Linear Least Squares')
        #self.cap = (alpha/np.sum(alpha,axis=1,keepdims=True)).reshape(self.cap.shape)
...
if __name__ == '__main__':
    ...
    dataset_dir = os.path.join(data_dir,f'{plant_type}/{city}/data/')
    log_dir = os.path.join(data_dir,f'{plant_type}/{city}/logs/model/')
    model_dir = os.path.join(data_dir,f'{plant_type}/{city}/model/')
    ...
    trees = lgb.train({
        ...
        'learning_rate': 0.01,
        'verbosity': -1,
        'n_estimators': n_estimators}, train_data)
